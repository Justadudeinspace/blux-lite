#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/flashing_helper.sh
...
ORIG="scripts/flashing_helper.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/flashing_helper.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/flashing_helper.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/flashing_helper.sh") ==="
