#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: plugin_menu.sh
...
ORIG="plugin_menu.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/plugin_menu.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "plugin_menu.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "plugin_menu.sh") ==="
