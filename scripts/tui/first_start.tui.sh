#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: first_start.sh
...
ORIG="first_start.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/first_start.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "first_start.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "first_start.sh") ==="
