#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/logs_rotate.sh
...
ORIG="scripts/logs_rotate.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/logs_rotate.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/logs_rotate.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/logs_rotate.sh") ==="
