#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/wifi_battery_info.sh
...
ORIG="scripts/wifi_battery_info.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/wifi_battery_info.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/wifi_battery_info.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/wifi_battery_info.sh") ==="
