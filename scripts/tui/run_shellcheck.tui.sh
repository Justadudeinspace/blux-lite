#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/run_shellcheck.sh
...
ORIG="scripts/run_shellcheck.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/run_shellcheck.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/run_shellcheck.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/run_shellcheck.sh") ==="
