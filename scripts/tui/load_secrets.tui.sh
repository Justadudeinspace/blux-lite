#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: examples/cloud/load_secrets.sh
...
ORIG="examples/cloud/load_secrets.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/examples/cloud/load_secrets.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "examples/cloud/load_secrets.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "examples/cloud/load_secrets.sh") ==="
