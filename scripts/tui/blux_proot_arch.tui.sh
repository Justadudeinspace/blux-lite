#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/blux_proot_arch.sh
...
ORIG="scripts/blux_proot_arch.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/blux_proot_arch.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/blux_proot_arch.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/blux_proot_arch.sh") ==="
