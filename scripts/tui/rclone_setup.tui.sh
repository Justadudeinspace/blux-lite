#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: examples/cloud/rclone_setup.sh
...
ORIG="examples/cloud/rclone_setup.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/examples/cloud/rclone_setup.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "examples/cloud/rclone_setup.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "examples/cloud/rclone_setup.sh") ==="
