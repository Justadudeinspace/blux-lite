#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: blux-lite.sh
...
ORIG="blux-lite.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/blux-lite.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "blux-lite.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "blux-lite.sh") ==="
