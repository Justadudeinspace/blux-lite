#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/runner_dry_run.sh
...
ORIG="scripts/runner_dry_run.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/runner_dry_run.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/runner_dry_run.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/runner_dry_run.sh") ==="
