#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/blux_freshtermux.sh
...
ORIG="scripts/blux_freshtermux.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/blux_freshtermux.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/blux_freshtermux.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/blux_freshtermux.sh") ==="
