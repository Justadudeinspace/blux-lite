#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/termux_boot_enable_blux.sh
...
ORIG="scripts/termux_boot_enable_blux.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/termux_boot_enable_blux.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/termux_boot_enable_blux.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/termux_boot_enable_blux.sh") ==="
