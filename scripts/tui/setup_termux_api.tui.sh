#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/setup_termux_api.sh
...
ORIG="scripts/setup_termux_api.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/setup_termux_api.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/setup_termux_api.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/setup_termux_api.sh") ==="
