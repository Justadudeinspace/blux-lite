#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/main_menu/logging.sh
...
ORIG="scripts/main_menu/logging.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/main_menu/logging.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/main_menu/logging.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/main_menu/logging.sh") ==="
