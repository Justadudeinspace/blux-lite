#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/remove_engs_and_mods.sh
...
ORIG="scripts/remove_engs_and_mods.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/remove_engs_and_mods.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/remove_engs_and_mods.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/remove_engs_and_mods.sh") ==="
