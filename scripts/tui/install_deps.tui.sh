#!/usr/bin/env bash
# BLUX Lite — TUI wrapper for: scripts/main_menu/install_deps.sh
...
ORIG="scripts/main_menu/install_deps.sh"
ORIG_ABS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/main_menu/install_deps.sh"
LOG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/$(basename "scripts/main_menu/install_deps.sh").log"
...
menu() {
  while true; do
    clear
    echo "=== BLUX TUI: $(basename "scripts/main_menu/install_deps.sh") ==="
