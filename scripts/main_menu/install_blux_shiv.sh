...
# Install shiv tool
if ! "$PYBIN" -m shiv --help >/dev/null 2>&1 && ! command -v shiv >/dev/null 2>&1; then
  say "[INFO] Installing 'shiv' (zipapp packager) for current user..."
  "$PYBIN" -m pip install --user --upgrade shiv || { err "Failed to install shiv"; exit 1; }
fi

# Ensure console entry exists: pyproject.toml should define [project.scripts] blux = "shiv.cli:main"
if ! grep -qE '^\s*blux\s*=\s*"shiv\.cli:main"' "${ROOT}/pyproject.toml" 2>/dev/null; then
...
"$PYBIN" -m shiv -p '/usr/bin/env python3' -c blux -o "${BIN_DIR}/${APP_NAME}" "${ROOT}" || {
  # Fallback: build a .pyz then move it
  warn "Direct build failed; trying fallback to blux.pyz then moving"
  "$PYBIN" -m shiv -p '/usr/bin/env python3' -c blux -o "${ROOT}/blux.pyz" "${ROOT}" || { err "Shiv build failed"; exit 1; }
  mv -f "${ROOT}/blux.pyz" "${BIN_DIR}/${APP_NAME}"
}
